...
import dev.geminileft.reducers.TwoPartACacheReducer;

public class TwoPartACacheTest extends Configured implements Tool {
	...
	private static final int CACHE_FILE_ARG_INDEX = 2;
	 ...
	 public static void main(String[] args) throws Exception {
	     int exitCode = ToolRunner.run(new Configuration(), new TwoPartACacheTest(), args);
	     System.exit(exitCode);
	 }
	 
	 public int run(String[] args) throws Exception {
	   Configuration conf = this.getConf();
	   Job job = Job.getInstance(conf, "TwoPartACacheTest");
	   job.addCacheFile(
			   new Path(String.format("%s"
				, args[CACHE_FILE_ARG_INDEX])).toUri());
	   job.setInputFormatClass(TextInputFormat.class);
	   job.setOutputFormatClass(TextOutputFormat.class);
	   job.setJarByClass(TwoPartACacheTest.class);
	   job.setMapperClass(TwoPartMapper.class);
	   job.setReducerClass(TwoPartACacheReducer.class);
